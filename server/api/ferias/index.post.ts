...
export default defineEventHandler(async (event) => {
  ...
  const { nome, email, periodos = [] } = body as {
    nome: string
    email: string
  ...
  const totalDias = periodos.reduce((acc: number, p) => {
    if (p.inicio && p.fim) {
      const diff = (new Date(p.fim).getTime() - new Date(p.inicio).getTime()) / (1000 * 60 * 60 * 24) + 1
      return acc + diff
    }
    return acc
  }, 0)
  ...
  if (totalDias > 30) {
    throw createError({ statusCode: 400, statusMessage: 'O total de férias não pode ultrapassar 30 dias' })
  }
